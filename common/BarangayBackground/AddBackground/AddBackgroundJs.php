    ...
    $(document).ready(function()
	{
        $("#btnUpload").click(function()
		{
            ...
            if(files.length > 0 )
            {
                ...
                $.ajax(
                {
                    url: "modules/BarangayBackground/AddBackground/Upload.php",
                    type: "POST",
                    data: fd,
                    contentType: false,
                    processData: false,
        ...
        $("#fBarangayBackground").submit(function(event) 
        {
            ...
            }).then((result) => 
            {
                if (result.isConfirmed)
                {
                    ...
                    {
                        type: "POST",
                        url: "modules/BarangayBackground/AddBackground/Verify.php",
                        data: 
                        {
                            brgycode: brgycode,
                            type: "VERIFYBARANGAYPROFILE"
                        },
                        success: function(result)
                        {
                            ...
                            if(result == "RecordNotExist")
                            {
                                $.ajax(
                                {
                                    type: "POST",
                                    url: "modules/BarangayBackground/AddBackground/Insert.php",
                                    data: 
                                    {
                                        brgycode: brgycode,
                                        profile: profile,
                                    ...
                                    success: function(result)
                                    {
                                        if(result == "RecordSaved")
                                        {
                                            ...
                                            {
                                                title: "Success!",
                                                text: "Barangay record has been saved successfully.",
                                                icon: "success",
                                            }).then(function(isConfirm)
                                            {
                                                location.reload();
