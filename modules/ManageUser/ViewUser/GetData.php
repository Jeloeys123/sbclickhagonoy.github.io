	...
	if(isset($_POST['type']))
	{
		...
		if($type == "GETALLACCOUNT")
		{
			...
			{   
				...
				$brgyRepresentativeQuery.= " AND (barangay_representative.EMPCODE LIKE '%".$requestData['search']['value']."%' OR barangay_mstr.BRGYNAME LIKE '%".$requestData['search']['value']."%' OR CONCAT(barangay_representative.FIRSTNAME,' ',barangay_representative.LASTNAME) LIKE '%".$requestData['search']['value']."%' OR CONCAT(barangay_representative.LASTNAME,' ',barangay_representative.FIRSTNAME) LIKE '%".$requestData['search']['value']."%' OR barangay_representative.EMAIL LIKE '%".$requestData['search']['value']."%' OR barangay_representative.MOBILE LIKE '%".$requestData['search']['value']."%' OR position_mstr.DESCRIPTION LIKE '%".$requestData['search']['value']."%')";
            }
			...
			$brgyRepresentativeQuery .= " ORDER BY barangay_mstr.BRGYNAME ASC,". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
			...
			if($brgyRepresentativeResult != NULL)
			{
				...
				{  
                    ...
                    //PREPARING AN ARRAY
					$nestedData = array(); 
					$nestedData[] = "<center style = 'text-align: left;'>".strtoupper($empcode)."</center>";
            ...
            $json_data = array(
				...
				"data"            => $usermasterlist_arr   // total data array
			);
